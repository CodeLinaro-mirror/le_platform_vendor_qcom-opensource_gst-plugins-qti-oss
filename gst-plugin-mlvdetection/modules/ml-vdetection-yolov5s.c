...
static const gfloat qscales[3] = { 0.094353646, 0.103818186, 0.120787375 };
      ...
      for (y = 0; y < maxheight; y++) {
        for (x = 0; x < maxwidth; x++, num += n_layers) {
          ...
          if (sar_n > sar_d) {
            ...
            prediction.top /= 416.0 / coeficient;
            prediction.bottom /= 416.0 / coeficient;
            prediction.left /= 416.0;
            prediction.right /= 416.0;
          } else if (sar_n < sar_d) {
            gdouble coeficient = 0.0;
            gst_util_fraction_to_double (sar_d, sar_n, &coeficient);

            prediction.top /= 416.0;
            prediction.bottom /= 416.0;
            prediction.left /= 416.0 / coeficient;
            prediction.right /= 416.0 / coeficient;
          }
